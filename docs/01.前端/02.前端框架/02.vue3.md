---
title: Java程序员要掌握的前端-vue3
date: 2023-02-11 19:14:56
permalink: /pages/d84568/
---
# Vue 3
...
![image-20220911090418621](https://xingqiu-tuchuang-1256524210.cos.ap-shanghai.myqcloud.com/5339/image-20220911090418621.png)

要对 *.vue 做语法支持，还要安装一个 Volar 插件

![image-20220911090756694](https://xingqiu-tuchuang-1256524210.cos.ap-shanghai.myqcloud.com/5339/image-20220911090756694.png)
...
![image-20220815141648040](https://xingqiu-tuchuang-1256524210.cos.ap-shanghai.myqcloud.com/5339/image-20220815141648040.png)
...
![image-20220906141650435](https://xingqiu-tuchuang-1256524210.cos.ap-shanghai.myqcloud.com/5339/image-20220906141650435.png)
...
* **reactive 只能将对象类型**变为【响应式】，对基本类型无效（例如 string，number，boolean）
...
* 双向绑定**只适用于表单这种带【输入】功能的标签**，其它标签的数据绑定，单向就足够了
...
* fullName 即为计算属性，**它具备`缓存`功能，即 firstName 和 lastName 的值发生了变化，才会重新计算**
* **如果用函数实现相同功能，则没有缓存功能**
...
这就要求我们`区分开发环境和生产环境`，这件事交给构建工具 vite 来做
...
请求能成功发送，那么得到的状态码总会是200，但是因为后端代码异常返回应该是走error部分

1. 后端返回的响应状态码总是200，用自定义错误码表示出错，这时会走响应拦截器第一个箭头函数，用 response.data.code 做分支判断
...
const {count, inc, dec} = useCounter(1，{min:xx,max:xxx})
...
//插槽 column获得当前列的信息，record当前行的信息
...
3.结构起别名const {data : MyData}=xxx
4.
    删除数据后进行查询，可能导致数据删除前，查询语句就执行了，导致两者的冲突

 5.
const { runAsync: deleteById } = useRequest<AxiosRespString, 
 async function onDeleteIds() {
  await deleteByIds(ids.value)
  ids.value = []
  search(dto.value)
...
```vue
<A4Save :id="id" :dto="saveDto" v-model:visible="saveVisible" @save='onSaved'></A4Save>
...
* **Object.assign 是将源对象（参数2）的属性值赋值给目标对象（参数1）的同名属性，效果等价于**
  ...
  object,assign(savaDto,record)
  
  
  
  子组件宏定义 define<{id:number}>()
  计算属性无法直接获得id
  const props=define<{id:number}>()
  const xx=computed(()=>props.id xxxx)
  ```
...
>()
const emit=defineEmits(['increase','update:title'])
emit('update:title',value) //v-modle:title=”xx"  绑定更改
...
//validateInfos 得到校验规则里面的message  与表单项进行绑定 v-bind="validateInfos.name"
//

await validate校验方法 提交前的校验方法,校验失败会抛出异常
...
![image-20220926145812121](https://xingqiu-tuchuang-1256524210.cos.ap-shanghai.myqcloud.com/5339/image-20220926145812121.png)

![image-20220926145959690](https://xingqiu-tuchuang-1256524210.cos.ap-shanghai.myqcloud.com/5339/image-20220926145959690.png)
...
![image-20220926150819624](https://xingqiu-tuchuang-1256524210.cos.ap-shanghai.myqcloud.com/5339/image-20220926150819624.png)
...
![image-20220926151216217](https://xingqiu-tuchuang-1256524210.cos.ap-shanghai.myqcloud.com/5339/image-20220926151216217.png)

访问 /a3/teacher 时，效果为

![image-20220926151249403](https://xingqiu-tuchuang-1256524210.cos.ap-shanghai.myqcloud.com/5339/image-20220926151249403.png)
...
![image-20220930172635166](https://xingqiu-tuchuang-1256524210.cos.ap-shanghai.myqcloud.com/5339/image-20220930172635166.png)
