---
title: Java程序员要掌握的前端-vue2
date: 2023-02-11 19:14:56
permalink: /pages/ea7a2b/
---
# 三. Vue 2
...
![image-20220815141136895](https://xingqiu-tuchuang-1256524210.cos.ap-shanghai.myqcloud.com/5339/image-20220815141136895.png)

选择手动配置项目

![image-20220815141312244](https://xingqiu-tuchuang-1256524210.cos.ap-shanghai.myqcloud.com/5339/image-20220815141312244.png)

添加 vue router 和 vuex

![image-20220815141412380](https://xingqiu-tuchuang-1256524210.cos.ap-shanghai.myqcloud.com/5339/image-20220815141412380.png)

选择版本，创建项目

![image-20220815141459878](https://xingqiu-tuchuang-1256524210.cos.ap-shanghai.myqcloud.com/5339/image-20220815141459878.png)
...
```vue
<template v-for='m1 of tp'>
	//通过判断有无子标签进行创建 以及设置唯一标识key
	<el-menu-item v-if='m1.children' :key='m1.id'></el-menu-item>
	<el-submenu v-else></el-submenu>
</template>
```
...
vuex 可以在多个组件之间共享数据，并且**共享的数据是【响应式】**的，**即数据的变更能及时渲染到模板**
...
* 类似的，调用 mutation 修改共享数据也可以简化
  ...
  * **mapMutations 返回的对象中包含的方法，就会调用 store.commit() 来执行 mutation 方法**
  ...
  `this.stroe.commit('mutations方法'，属性)`
* 注意参数传递略有不同
...
mutations 方法内**不能包括修改不能立刻生效的代码**，否则会造成 **Vuex 调试工具工作不准确**，必须把这些代码写在 actions 方法中
...
* 登录成功会优先跳转至 this.redirect 路径、否则跳转至 /。记录注销之前的界面路径
...
```vue
try {
const req=await this.$store.dispatch('user/login', this.loginForm)
this.$router.push({ path: this.redirect || '/', query: this.otherQuery })
this.loading = false
...
* `login({ commit }, userInfo)` ，action函数默认自带参数context，{commit}就从context里面解构出commit
...
![image-20220827194047788](https://xingqiu-tuchuang-1256524210.cos.ap-shanghai.myqcloud.com/5339/image-20220827194047788.png)
...
修改模块 如果用this.change=row,使得change和row指向同一个对象，那么当row进行修改时，表单的数据也会跟着修改
